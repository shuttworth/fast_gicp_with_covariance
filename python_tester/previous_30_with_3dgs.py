...
def make_pointcloud_from_img(depth_img, rgb_img,
	...
	filter = np.where((depth_img_flatten != 0) &
	                  (depth_img_flatten <= depth_trunc))
...
def main():
    if len(sys.argv) < 5:
        print(
            'usage: previous_30_with_3dgs.py [dataset_path] [tum or replica] [downsample_resolution] [visualize?]')
        return
    # List input files
    ...
    if sys.argv[2] == "tum":
        ...
        print(
            'usage: gicp_odometry.py [dataset_path] ["tum" or "replica"] [downsample_resolution] [visualize?]')
        return
    if sys.argv[3] != "false":
    ...
    pointclouds_stack = []
    if mode == "replica":
        filenames = sorted([seq_path + '/results/' +
                           x for x in os.listdir(seq_path+'/results/') if x.endswith('.png')])
        ...
        filenames = sorted(
           [seq_path + '/depth/' + x for x in os.listdir(seq_path+'/depth/') if x.endswith('.png')])
    ...
    for i, filename in enumerate(filenames):
        ...
        # points = pygicp.downsample(points, downsample_resolution)
            selected_indices = np.random.choice(len(points), size=(
                int)(len(points)*downsample_resolution), replace=False)
        ...
        if (i % 30 == 0) or (i == len(filenames)-1):
            pyplot.clf()
            pyplot.title(
                      f'Downsample ratio {downsample_resolution}\nfps : {fps:.2f}')
            pyplot.plot(traj[:, 0], traj[:, 1],
                        label='g-icp trajectory', linewidth=3)
            pyplot.legend()
            pyplot.plot(gt_traj_vis[:, 0], gt_traj_vis[:, 1],
